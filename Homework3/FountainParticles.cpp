...
void FountainParticles::generate(int maxNewParticles)
{
	if (maxNewParticles + nbrOfParticles < maxParticles) { // generate the particles
		for (int currentParticle = 0; currentParticle < maxNewParticles; ++currentParticle) {
			float xLoc = -5.0 + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (5.0 - -5.0)));
			float zLoc = -5.0 + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (5.0 - -5.0)));
			positions[(nbrOfParticles + currentParticle) * 3] = xLoc;
			positions[(nbrOfParticles + currentParticle) * 3 + 1] = 12.0f;
			positions[(nbrOfParticles + currentParticle) * 3 + 2] = zLoc;
			...
			accelerations[(nbrOfParticles + currentParticle) * 3 + 1] = -10.0;
